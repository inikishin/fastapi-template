...
class <Entity>(IDMixin, TimestampMixin, Base):
    ...
    related: Mapped["<RelatedEntity>"] = relationship(back_populates="<entities>")
```

- `Base` already exists — do not redeclare it.
- `__tablename__` is snake_case and usually plural.
- Every FK has `index=True` and a named constraint.
- Use `Mapped[T] = mapped_column(...)` for all columns — this is the SQLAlchemy 2.0 style and provides full type annotations for mypy/IDE. Never use the legacy `Column()` approach.
- Optional fields: `Mapped[Optional[T]] = mapped_column(..., nullable=True)`.
- List relationships: `Mapped[List["<Entity>"]] = relationship(...)`.
- `TimestampMixin` is used by default for live business entities. Omit it for lookup tables (dictionaries, enums) that don't need audit columns.
...
- **Always use `Mapped[T] = mapped_column(...)` — SQLAlchemy 2.0 style.** Never use the legacy `name = Column(...)` form. This applies to model files and to mixins alike.
...
- Enum columns use `sa.Enum(MyEnum, name="<enum_name>")`. Without `name=` PostgreSQL assigns an autogenerated name and the migration suffers.
- Nullable columns: `Mapped[Optional[T]] = mapped_column(..., nullable=True)`. Non-nullable: `Mapped[T] = mapped_column(..., nullable=False)`.
- Arrays: `Mapped[List[str]] = mapped_column(ARRAY(String))`. JSON: `JSONB` (from `sqlalchemy.dialects.postgresql`).
